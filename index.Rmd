...
2. Informacional (incentivos à especialização);
3. Partidária (extensão do jogo legislativo).

## Distributivista e Informacional

Parâmetros incompatíveis:

   1. Ausência de duas instituições informais no caso brasileiro: _seniority_ e auto-seleção;
   2. Ausência do controle total de agenda e alta rotatividade;
...
>- _Drawbacks_: dependência entre os casos, variável dep. fixa exógenamente e chance condicional às chances das demais observações.
...
2. Comissões importantes frequentemente possuem membros pouco disciplinados;
3. Desagregando por partidos, poucos padrões nítidos aparecem, mas existe muita variação entre eles;
